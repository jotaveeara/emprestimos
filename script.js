(() => {
  ...
  function bindEvents(){
    ...
    ["loan-plan","loan-amount","loan-amount-custom","weekly-term","monthly-count","monthly-installment"].forEach(id=>$(id).addEventListener("input",()=>{if(id==="loan-plan")updatePlanFields();renderPreview()}));
  ...
  function loanPrincipal(){return Number($("loan-plan").value==="monthly"?$("loan-amount-custom").value:$("loan-amount").value)||0}
  function updatePlanFields(){const type=$("loan-plan").value,isMonthly=type==="monthly";$("weekly-term-wrap").hidden=type!=="weekly";$("monthly-count-wrap").hidden=!isMonthly;$("monthly-installment-wrap").hidden=!isMonthly;$("loan-amount-preset-wrap").hidden=isMonthly;$("loan-amount-custom-wrap").hidden=!isMonthly;$("loan-amount").disabled=isMonthly;$("loan-amount-custom").disabled=!isMonthly;if(!isMonthly){const amounts=Object.keys(type==="daily"?DAILY:WEEKLY);const old=$("loan-amount").value;$("loan-amount").innerHTML=amounts.map(a=>`<option value="${a}">${money(a)}</option>`).join("");if(amounts.includes(old))$("loan-amount").value=old}renderPreview()}
  function loanTerms(){const type=$("loan-plan").value,principal=loanPrincipal();if(type==="daily")return {count:20,amount:DAILY[principal],interval:"daily"};if(type==="weekly"){const count=Number($("weekly-term").value);return {count,amount:WEEKLY[principal]?.[count],interval:"weekly"}}return {count:Number($("monthly-count").value),amount:Number($("monthly-installment").value),interval:"monthly"}}
